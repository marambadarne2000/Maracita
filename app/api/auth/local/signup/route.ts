...
export async function POST(request: Request) {
  ...
  const key = `signup:${email || 'invalid'}`;
  const rate = await consumeAuthAttempt(database, key, { limit: 5, windowMinutes: 15 });
  if (!rate.allowed) return Response.json({ error: 'Too many account requests. Please wait before trying again.' }, { status: 429, headers: { 'Retry-After': String(rate.retryAfterSeconds) } });
